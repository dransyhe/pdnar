...
from src.dataset.algorithms.vertex_cover_solver import compute_from_nxgraph


def vertex_cover(graph, eps=0.1):  
    B = copy.deepcopy(graph) 

    # Get optimal vertex covers 
    primal_optimal_solution, primal_optimal_weight = compute_from_nxgraph(B)
    primal_optimal_solution = primal_optimal_solution[0].clone().long()
    ...
    while any(edge_mask):
        ...
        for e in range(num_edges):
            if not edge_mask[e]:
                continue
            u, v = B.neighbors(num_nodes + e)
            delta[e] = min(w_p[u] / d_p[u], w_p[v] / d_p[v])
...
if __name__ == '__main__':
    from src.dataset.graph import generate_ordinary_bipartite_graphs

    B = generate_ordinary_bipartite_graphs(1, 10)[0]
    data = vertex_cover(B)
    breakpoint()